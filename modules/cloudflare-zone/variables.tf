...
variable "settings" {
  description = "Cloudflare zone settings to configure. Only specify settings you want to override from defaults or add additional settings."
  type        = map(any)
  default     = {}

  # Default settings applied by this module (can be overridden):
  ...
  #
  # Additional settings you can add (availability depends on your Cloudflare plan):
  # tls_1_3                  = "on"          # on, off, zrt (zero round trip)
  # challenge_ttl            = 1800          # 300-31536000 seconds
  # privacy_pass             = "on"          # on, off
  # brotli                   = "on"          # on, off (often read-only on free plans)
  # early_hints              = "off"         # on, off
  # http2                    = "on"          # on, off (often read-only)
  # http3                    = "on"          # on, off (often read-only)
  ...
  #
  # Note: Some settings may be read-only depending on your Cloudflare plan.
  # If you encounter "Not allowed to edit setting" errors, remove those settings from your configuration.
}
